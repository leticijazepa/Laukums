...
function laukums(m1,m2,m3){
    const pusper = perimetrs(m1,m2,m3)/2;
    let zlaukums = Math.sqrt(pusper*((pusper-m1) * (pusper-m2)* (pusper-m3)));
    console.log(zlaukums);
    return zlaukums;
}
//Funkcija,kura parbauda vai eksiste trijsturis ar nolasitajam malam function...
//parbauda vai izpildas trijstura nevienadiba
//atgreiz true vai false
...
console.log(m1,m2,m3);
if(!nolasa()){
    t="Nekorekti ievaddati, trījstūra malu vērtībām ir jābūt lielākām par 0";
}else{
    if(trijsturis(m1,m2,m3)==false){
        t="Trijstūris neeksistē, jo jebkuru 2 malu garumu summai ir jābūt lielākai par trešo malu!";
    }else{
        t="Trījstūris ar malu garumiem "+m1+", "+m2+" un "+m3+" eksistē. "
        if(m1==m2 && m2==m3){
...
function izvadaTekstu(){
    ...
    //elementā ar id izvada izvada tekstu
    console.log(teksts);
